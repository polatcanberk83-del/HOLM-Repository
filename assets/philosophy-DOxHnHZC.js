import{$t as e,B as t,C as n,D as r,Et as i,Ht as a,I as o,It as s,N as c,Pt as l,S as u,Tt as d,Vt as f,an as p,dt as m,et as h,f as g,h as ee,ht as te,i as _,lt as ne,m as re,n as v,nn as y,o as ie,on as ae,p as oe,qt as b,r as x,rn as se,s as ce,t as le,tn as S,ut as C,wt as ue,x as de,zt as w}from"./pageTransition-C1rQ-lDC.js";import{t as T}from"./UnrealBloomPass-DTcgmD1s.js";import{n as E,t as D}from"./holm new logo-JiLmqs7J.js";var O=.16,k=.09,A=.18,j=typeof window<`u`&&(window.innerWidth<768||`ontouchstart`in window)?.42:.5,M=22,N=.3,P=.985,F=.9,I=.92,L=1.6,R=.72,z=1.4,B=`#000000`,V=`#02040c`,H=`#06183a`,fe=`#123063`,U=`
varying vec2 vUv;
void main() {
  vUv = uv;
...
`;function W(e){return new y(parseInt(e.slice(1,3),16)/255,parseInt(e.slice(3,5),16)/255,parseInt(e.slice(5,7),16)/255)}var G=5.6,he=4.4,ge=.95,_e=.75,K=[{x:0,y:0,scale:1.2,side:null,hasText:!1},{x:.34,y:.03,scale:1,side:`left`,hasText:!0},{x:-.34,y:-.05,scale:1.24,side:`right`,hasText:!0},{x:0,y:.08,scale:.62,side:`left`,hasText:!0},{x:-.3,y:.06,scale:1.18,side:`right`,hasText:!0},{x:.28,y:-.04,scale:1.28,side:`left`,hasText:!0},{x:0,y:.02,scale:1.35,side:`center-below`,hasText:!0}],ve=1,ye=16773852,be=9023716,xe=12108014,Se=.012,q=1.05;`${Se.toFixed(4)}${q.toFixed(4)}${q.toFixed(4)}${.35.toFixed(4)}${320 .toFixed(1)}`;var J=[[`Some studios tell stories.`,`We work toward a single moment.`],[`The moment a rough idea holds still —`,`and becomes something finished.`],[`It is rare. It forms under pressure.`,`The way carbon becomes a diamond.`],[`So that is the shape we keep returning to.`,`Not decoration. A reminder of what we are after.`],[`Between sketch and masterpiece,`,`there is patience.`],[`Ready to create your moment.`]];function Ce(e){return e.split(` `).map(e=>e?`<span class="holm-philosophy__word">${Array.from(e).map(e=>`<span class="holm-philosophy__char">${e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`)}</span>`).join(``)}</span>`:``).join(` `)}var Y=`Let's talk`,we=`/contact/`,Te=class _{constructor({lenis:e}={}){this.lenis=e||null,this._reducedMotion=window.matchMedia(`(prefers-reduced-motion: reduce)`).matches,this._isMobile=window.innerWidth<768||`ontouchstart`in window,this.renderer=null,this.scene=null,this.camera=null,this.diamond=null,this.envMap=null,this._lights=[],this._heroPlane=null,this.container=null,this.canvas=null,this.blocks=[],this._rafId=null,this._active=!1,this._prevTime=0,this._scrollT=0,this._idleSpin=0,this._ctaEl=null,this._ctaMove=null,this._ctaLeave=null,this._ctaSetX=null,this._ctaSetY=null,this._ctaInnerSetX=null,this._ctaInnerSetY=null,this._targetPos=new y,this._targetScale=new y(1,1,1),this._targetTilt=0,this._pressure=0,this._mouseNdc=new S(-99,-99),this._mouseWorld=new y,this._mouseWorldSmooth=new y(999,999,999),this._invMat=new ne,this._pointerActive=!1,this._elapsed=0,this._fluidQuadCam=null,this._fluidQuadScene=null,this._fluidQuadMesh=null,this._fluidMat=null,this._displayMat=null,this._fluidTargetA=null,this._fluidTargetB=null,this._displayTarget=null,this._fluidFrame=0,this._mousePx=new S(0,0),this._prevMousePx=new S(0,0),this._lastMouseMoveMs=0,this._onResize=this._onResize.bind(this),this._onLenisScroll=this._onLenisScroll.bind(this),this._onNativeScroll=this._onNativeScroll.bind(this),this._onPointerMove=this._onPointerMove.bind(this),this._observer=null}init(){this._createDOM(),this._createThree(),this._bindScroll(),this._bindResize(),this._bindPointer(),this._observeBlocks(),this._bindCtaMagnetic(),this._startLoop()}_bindPointer(){window.addEventListener(`pointermove`,this._onPointerMove,{passive:!0})}_onPointerMove(e){if(this._mouseNdc.set(e.clientX/window.innerWidth*2-1,-(e.clientY/window.innerHeight*2-1)),this._pointerActive=!0,this._fluidMat){window.innerWidth;let t=window.innerHeight,n=e.clientX*j,r=(t-e.clientY)*j;this._prevMousePx.copy(this._mousePx),this._mousePx.set(n,r),this._lastMouseMoveMs=performance.now(),this._fluidMat.uniforms.iMouse.value.set(this._mousePx.x,this._mousePx.y,this._prevMousePx.x,this._prevMousePx.y)}}destroy(){this._active=!1,this._rafId&&cancelAnimationFrame(this._rafId),window.removeEventListener(`resize`,this._onResize),window.removeEventListener(`pointermove`,this._onPointerMove),this.lenis&&this._lenisScrollBound?this.lenis.off(`scroll`,this._onLenisScroll):window.removeEventListener(`scroll`,this._onNativeScroll),this._observer&&this._observer.disconnect(),this._ctaMove&&window.removeEventListener(`mousemove`,this._ctaMove),this._ctaLeave&&window.removeEventListener(`mouseleave`,this._ctaLeave),this.diamond&&this.diamond.material.dispose(),this._heroPlane&&(this._heroPlane.geometry.dispose(),this._heroPlane.material.uniforms?.uTextMap?.value?.dispose(),this._heroPlane.material.dispose()),this._causticPlane&&(this._causticPlane.geometry.dispose(),this._causticPlane.material.dispose()),this.envMap&&this.envMap.dispose();for(let e of this._lights)this.scene?.remove(e);this._fluidTargetA?.dispose(),this._fluidTargetB?.dispose(),this._displayTarget?.dispose(),this._fluidMat?.dispose(),this._displayMat?.dispose(),this._fluidQuadMesh?.geometry?.dispose(),this.renderer&&(this.renderer.dispose(),this.renderer.forceContextLoss?.()),this.container?.parentNode&&this.container.parentNode.removeChild(this.container),this.renderer=this.scene=this.camera=null,this.diamond=this.envMap=null,this.container=this.canvas=null}_createDOM(){let e=document.createElement(`div`);e.className=`holm-philosophy`,this._reducedMotion&&e.classList.add(`is-reduced-motion`);let t=0;if(e.innerHTML=`
    ...
    `,document.body.appendChild(e),this.container=e,this.canvas=e.querySelector(`.holm-philosophy__canvas`),this.blocks=[...e.querySelectorAll(`.holm-philosophy__beat`)],this._beatEls=[],e.querySelectorAll(`[data-beat]`).forEach(e=>{let t=parseInt(e.dataset.beat,10);Number.isFinite(t)&&K[t]&&this._beatEls.push({el:e,beat:K[t]})}),!this._reducedMotion){let t=e.querySelectorAll(`.holm-philosophy__char`);g.set(t,{yPercent:115,opacity:0})}}_createThree(){this.renderer=new re({canvas:this.canvas,antialias:!0,alpha:!1,powerPreference:`high-performance`}),this.renderer.setPixelRatio(Math.min(window.devicePixelRatio,this._isMobile?1:2)),this.renderer.setSize(window.innerWidth,window.innerHeight),this.renderer.toneMapping=4,this.renderer.toneMappingExposure=.75,this.renderer.outputColorSpace=w,this.scene=new f,this._initFluid(),this.scene.background=this._displayTarget.texture,this.camera=new ue(36,window.innerWidth/window.innerHeight,.1,100),this.camera.position.set(0,0,G),this.camera.lookAt(0,0,0);let e=new oe(this.renderer),t=this._buildStudioEnvScene();this.envMap=e.fromScene(t,.02).texture,t.traverse(e=>{e.geometry&&e.geometry.dispose(),e.material&&e.material.dispose()}),e.dispose(),this.scene.environment=this.envMap,this.scene.environmentIntensity=1.5,this._composer=new ce(this.renderer),this._composer.setPixelRatio(this.renderer.getPixelRatio()),this._composer.setSize(window.innerWidth,window.innerHeight),this._composer.addPass(new ie(this.scene,this.camera)),this._isMobile||(this._bloom=new T(new S(window.innerWidth,window.innerHeight),.18,.22,.92),this._composer.addPass(this._bloom)),this._composer.addPass(new E),this._isMobile||this._createCausticPlane(),this._createDiamond(),this._isMobile||this._createHeroPlane(),this._addLights()}_buildStudioEnvScene(){let e=new f,t=new b(30,24,12),n=new a({side:1,vertexShader:`
        varying vec3 vP;
        void main() {
          vP = position;
          gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
      ...
      `});e.add(new C(t,n));let r=(t,n,r,i,a,o)=>{let s=new b(o,12,8),c=new m({color:i});c.color.multiplyScalar(a);let l=new C(s,c);l.position.set(t,n,r),e.add(l)};return r(7,6,5,16777215,45,.85),r(-6,3,4,16185599,24,1.2),r(0,6,-7,16777215,30,.9),r(5,-2,4,16767136,14,.45),r(-6,-1,3,11061503,14,.45),r(-11,1,0,16777215,65,.2),r(11,1,0,16777215,65,.2),r(0,-3,9,16777215,40,.3),e}_createCausticPlane(){let e=new C(new i(5,5,1,1),new a({transparent:!0,depthWrite:!1,blending:2,uniforms:{uTime:{value:0},uIntensity:{value:.55}},vertexShader:`
        varying vec2 vUv;
        void main() {
          vUv = uv;
          gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
      ...
      `}));e.position.z=-1.2,e.renderOrder=-1,this._causticPlane=e,this.scene.add(e)}_createHeroPlane(){let e=document.createElement(`canvas`);e.width=2560,e.height=640;let t=e.getContext(`2d`);t.fillStyle=`rgba(255, 255, 255, 1.0)`,t.font=`italic 300 480px 'Fraunces', 'Times New Roman', serif`,t.textAlign=`center`,t.textBaseline=`middle`,t.fillText(`philosophy`,e.width/2,e.height/2+30);let r=new n(e);r.colorSpace=w,r.minFilter=h,r.magFilter=h,r.anisotropy=this.renderer.capabilities.getMaxAnisotropy();let o=new a({vertexShader:`
        varying vec2 vUv;
        void main() {
          vUv = uv;
          gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
      ...
      `,uniforms:{uTextMap:{value:r},uFluidMap:{value:this._displayTarget.texture},uTime:{value:0},uMouseUv:{value:new S(-2,-2)},uCursorFocus:{value:0},uOpacity:{value:1}},transparent:!0,depthWrite:!1}),c=G- -8,l=this.camera.fov*Math.PI/360,u=Math.tan(l)*c*this.camera.aspect,f=Math.max(u*2*1.15,20),p=f/(e.width/e.height),m=this._isMobile?[32,8]:[96,24],g=new i(f,p,m[0],m[1]);this._heroPlane=new C(g,o),this._heroPlane.position.z=-8,this._heroPlane.renderOrder=-1,this._heroPlaneSize={w:f,h:p,z:-8},this._heroPlaneMath=new d(new y(0,0,1),8),this._heroRaycaster=new s,this._heroHitPoint=new y,this.scene.add(this._heroPlane)}_initFluid(){let n=Math.max(2,Math.floor(window.innerWidth*j)),r=Math.max(2,Math.floor(window.innerHeight*j)),o={minFilter:h,magFilter:h,format:l,type:t,depthBuffer:!1,stencilBuffer:!1};this._fluidTargetA=new p(n,r,o),this._fluidTargetB=new p(n,r,o),this._displayTarget=new p(n,r,{...o,type:e}),this._fluidCurrent=this._fluidTargetA,this._fluidPrev=this._fluidTargetB,this._fluidMat=new a({vertexShader:U,fragmentShader:pe,defines:{FLUID_ITER:this._isMobile?4:8},uniforms:{iTime:{value:0},iResolution:{value:new S(n,r)},iMouse:{value:new se(0,0,0,0)},iFrame:{value:0},iPreviousFrame:{value:null},uBrushSize:{value:M},uBrushStrength:{value:N},uFluidDecay:{value:P},uTrailLength:{value:F},uStopDecay:{value:I}}}),this._displayMat=new a({vertexShader:U,fragmentShader:me,uniforms:{iTime:{value:0},iResolution:{value:new S(n,r)},iFluid:{value:null},uDistortionAmount:{value:L},uColor1:{value:W(B)},uColor2:{value:W(V)},uColor3:{value:W(H)},uColor4:{value:W(fe)},uColorIntensity:{value:R},uSoftness:{value:z}}}),this._fluidQuadCam=new u,this._fluidQuadScene=new f,this._fluidQuadMesh=new C(new i(2,2),this._fluidMat),this._fluidQuadScene.add(this._fluidQuadMesh),this._displayTarget.texture.colorSpace=w}_stepFluid(e){if(!this._fluidMat)return;if(this._isMobile&&(this._fluidFrame&1)==1){this._fluidFrame++;return}performance.now()-this._lastMouseMoveMs>100&&this._fluidMat.uniforms.iMouse.value.set(0,0,0,0),this._fluidMat.uniforms.iTime.value=e,this._fluidMat.uniforms.iFrame.value=this._fluidFrame,this._fluidMat.uniforms.iPreviousFrame.value=this._fluidPrev.texture,this._fluidQuadMesh.material=this._fluidMat,this.renderer.setRenderTarget(this._fluidCurrent),this.renderer.clear(),this.renderer.render(this._fluidQuadScene,this._fluidQuadCam),this._displayMat.uniforms.iTime.value=e,this._displayMat.uniforms.iFluid.value=this._fluidCurrent.texture,this._fluidQuadMesh.material=this._displayMat,this.renderer.setRenderTarget(this._displayTarget),this.renderer.clear(),this.renderer.render(this._fluidQuadScene,this._fluidQuadCam),this.renderer.setRenderTarget(null);let t=this._fluidCurrent;this._fluidCurrent=this._fluidPrev,this._fluidPrev=t,this._fluidFrame++}_createDiamond(){let e=this._isMobile?12:16,t=_._getBrilliantGeometry(e),n=this._isMobile,i=new te({color:16777215,metalness:0,roughness:n?.02:0,transmission:1,thickness:n?1:2.5,ior:2.417,attenuationDistance:12,attenuationColor:new r(16777215),envMapIntensity:n?1.5:2.8,iridescence:n?0:.25,iridescenceIOR:1.55,iridescenceThicknessRange:[400,900],clearcoat:+!n,clearcoatRoughness:0,transparent:!0,side:0});!n&&`dispersion`in i&&(i.dispersion=6.5),n||(i.userData.uEdgeTime={value:0},i.onBeforeCompile=e=>{e.uniforms.uEdgeTime=i.userData.uEdgeTime,e.vertexShader=e.vertexShader.replace(`#include <common>`,`#include <common>
